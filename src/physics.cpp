...
bool CControl::CheckTreeCollisions (const TVector3d& pos, TVector3d *tree_loc, ETR_DOUBLE *tree_diam) {
	...
	static ETR_DOUBLE last_collision_tree_diam = 0;
...
void CControl::AdjustTreeCollision (const TVector3d& pos, TVector3d *vel) {
	...
	if (CheckTreeCollisions (pos, &treeLoc, &tree_diam)) {
		...
		ETR_DOUBLE speed = vel->Norm();
		speed *= 0.8;  // original 0.7

		ETR_DOUBLE costheta = DotProduct (*vel, treeNml);
		if (costheta < 0) {
			ETR_DOUBLE factor;
...
/// -----------------------------------------------------------
	...
	ETR_DOUBLE disp_y = cpos.y + TUX_Y_CORR;
	shape->ResetNode (0);
...
TVector3d CControl::CalcRollNormal (ETR_DOUBLE speed) {
	TVector3d vel = ProjectToPlane (ff.surfnml, ff.vel);
	vel.Norm();

	ETR_DOUBLE roll_angle = MAX_ROLL_ANGLE;
	if (is_braking) roll_angle = BRAKING_ROLL_ANGLE;

	ETR_DOUBLE angle = turn_fact * roll_angle *
...
const ETR_DOUBLE airlog[]  = {-1, 0, 1, 2, 3, 4, 5, 6};
const ETR_DOUBLE airdrag[] = {2.25, 1.35, 0.6, 0, -0.35, -0.45, -0.33, -0.9};
...
TVector3d CControl::CalcAirForce () {
	...
	ETR_DOUBLE windspeed = windvec.Length();
	ETR_DOUBLE re = 34600 * windspeed;
	int tablesize = sizeof (airdrag) / sizeof (airdrag[0]);
	ETR_DOUBLE interpol = LinearInterp (airlog, airdrag, log10 (re), tablesize);
	ETR_DOUBLE dragcoeff = pow (10.0, interpol);
	ETR_DOUBLE airfact = 0.104 * dragcoeff *  windspeed;
	return airfact * windvec;
}

TVector3d CControl::CalcSpringForce () {
	ETR_DOUBLE springvel = DotProduct (ff.vel, ff.rollnml);
	ETR_DOUBLE springfact = min (ff.compression, 0.05) * 1500;
	springfact += clamp (0.0f, ff.compression - 0.05, 0.12) * 3000;
	springfact += max (0.0f, ff.compression - 0.12 - 0.05) * 10000;
...
TVector3d CControl::CalcFrictionForce (ETR_DOUBLE speed, const TVector3d& nmlforce) {
	if ((cairborne == false && speed > minFrictspeed) || g_game.finish) {
		ETR_DOUBLE fric_f_mag = nmlforce.Length() * ff.frict_coeff;
		fric_f_mag = min (MAX_FRICT_FORCE, fric_f_mag);
		TVector3d frictforce = fric_f_mag * ff.frictdir;

		ETR_DOUBLE steer_angle = turn_fact * MAX_TURN_ANGLE;
...
TVector3d CControl::CalcBrakeForce (ETR_DOUBLE speed) {
	if (g_game.finish == false) {
...
TVector3d CControl::CalcPaddleForce (ETR_DOUBLE speed) {
	...
	if (is_paddling) {
		if (cairborne) {
			...
			ETR_DOUBLE factor = -1 * min(MAX_PADD_FORCE, MAX_PADD_FORCE
...
TVector3d CControl::CalcNetForce (const TVector3d& pos, const TVector3d& vel) {
	...
	ff.frictdir = ff.vel;
	ETR_DOUBLE speed = ff.frictdir.Norm();
	ff.frictdir *= -1.0;

	static vector<ETR_DOUBLE> surfweights;
...
ETR_DOUBLE CControl::AdjustTimeStep (ETR_DOUBLE h, const TVector3d& vel) {
...
void CControl::SolveOdeSystem () {
	...
	while (!done) {
		...
		t = t + h;
		ETR_DOUBLE speed = new_vel.Length();
...
//		if (g_game.finish) new_vel = ScaleVector (0.99,new_vel);
	...
	ETR_DOUBLE step = (cpos - last_pos).Length();
	way += step;
}
