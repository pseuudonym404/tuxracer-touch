...
void CScore::Enter() {
	...
	frametop = AutoYPosN (Winsys.resolution.width > Winsys.resolution.height ? 20 : 32);
	area = AutoAreaN (30, 80, framewidth);
	FT.AutoSizeN (3);
	linedist = FT.AutoDistanceN (1);
	listtop = AutoYPosN (Winsys.resolution.width > Winsys.resolution.height ? 32 : 44);
	dd1 = 50 * Winsys.scale;
	...
	ResetGUI ();
	course = AddUpDown(area.right - 85, frametop, 0, (int)Course.CourseList.size()-1, 0);
