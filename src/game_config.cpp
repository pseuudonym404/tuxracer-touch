...
void LoadConfigFile () {
	...
	for (size_t i=0; i<list.Count(); i++) {
		...
		param.orient = SPIntN (line, "orientation", -1);
		param.sensit = SPIntN (line, "sensitivity", 10);
...
void SaveConfigFile () {
	...
	AddComment (liste, "Video orientation [0...3]");
	AddIntItem (liste, "orientation", param.orient);
	liste.AddLine();

	AddComment (liste, "Tilt sensitivity [1...20]");
	AddIntItem (liste, "sensitivity", param.sensit);
	liste.AddLine();
