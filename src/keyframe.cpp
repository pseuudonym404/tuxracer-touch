...
void CKeyframe::Init (const TVector3d& ref_position, ETR_DOUBLE height_correction) {
	if (!loaded) return;
...
void CKeyframe::Init (const TVector3d& ref_position, ETR_DOUBLE height_correction, CCharShape *shape) {
	if (!loaded) return;
