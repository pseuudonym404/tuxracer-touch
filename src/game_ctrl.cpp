...
void CPlayers::DelPlayer(size_t player) {
	for (size_t i = player; i < plyr.size() - 1; ++i)
		plyr[i] = plyr[i + 1];
	plyr.resize(plyr.size() - 1);
	if (player == plyr.size()) g_game.start_player = player - 1;
	g_game.start_character = plyr[player].character;
}
...
TTexture* CPlayers::GetPlayerAvatarTexture (size_t player) const {
	if (player >= plyr.size()) return 0;
	return plyr[player].avatar->texture;
}
