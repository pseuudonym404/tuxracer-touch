...
void CEvent::Enter () {
	...
	framewidth = 500;
	frametop = AutoYPosN (Winsys.resolution.width > Winsys.resolution.height ? 30 : 45);
	area = AutoAreaN (Winsys.resolution.width > Winsys.resolution.height ? 15 : 30, 80, framewidth);
	messtop = AutoYPosN (Winsys.resolution.width > Winsys.resolution.height ? 35 : 50);
	messtop2 = AutoYPosN (Winsys.resolution.width > Winsys.resolution.height ? 45 : 60);
	bonustop = AutoYPosN (Winsys.resolution.width > Winsys.resolution.height ? 20 : 35);
	...
	ETR_DOUBLE len = FT.GetTextWidth (Trans.Text(13));
...
//			0, colMBackgr, colBlack, 0.2);
	...
	if (ready == 0) {			// cup not finished
		...
		FT.DrawString (CENTER, AutoYPosN (Winsys.resolution.width > Winsys.resolution.height ? 6 : 25), ecup->name);
